...
# 加载BM25语义匹配模块
try:
    from .bm25_utils import intent_bm25
    BM25_ENABLED = True
...
def _cn_integer_to_num(cn_str: str) -> int:
    ...
    for unit in ['亿', '万']:
        if unit in cn_str:
            ...
            # 如"一万二"：parts[1]是"二"，直接乘以1000
            if len(parts) > 1 and parts[1]:
                right_part = parts[1].strip()
                if len(right_part) == 1 and right_part in CN_NUM and CN_NUM[right_part] < 10:
...
class NeuronCore:
    """语义识别核心类
    依赖抽象接口，所有数据操作通过DAL层实现
    """
    def __init__(self, dal: BaseDAL, net_adapter: BaseNetworkAdapter):
        """
        初始化语义识别核心
        :param dal: 数据访问层适配实例
        :param net_adapter: 网络适配层实例
        """
        self.dal = dal
        self.net_adapter = net_adapter
        self._load_neurons()
        logger.info(f"语义识别核心初始化完成，加载意图神经元数量：{len(self.intent_neurons)}")
    
    def _load_neurons(self):
        """加载所有意图神经元和突触权重"""
        # 从DAL层加载神经元和权重
        self.intent_neurons = self.dal.get_all_intent_neurons()
        self.synapse_weights = {}
        for neuron in self.intent_neurons:
            intent_id = neuron["intent_id"]
            self.synapse_weights[intent_id] = self.dal.get_synapse_weights_by_intent(intent_id)
        ...
        try:
            ...
            for scene_id, scene_config in scene_configs.items():
                ...
                for intent_config in intents:
                    ...
                    exists = False
                    try:
                        with self.dal.get_connection(write=False) as conn:
                            cursor = conn.cursor()
    ...
    def calculate_activation_scores(self, text: str, user_id: str = "global") -> Dict[str, float]:
        ...
        for neuron in self.intent_neurons:
            ...
            if weights is None:
                ...
                # 个性化权重优先
                if user_id != "global":
                    user_weights = self.dal.get_synapse_weights_by_intent(intent_id, user_id)
    ...
    def _match_general_intents(self, text: str) -> Dict[str, float]:
        ...
        intent_keyword_map = self.dal.get_intent_keyword_cache()
    ...
    def recognize_intent(self, text: str, user_id: str = "global", enable_multi_intent: bool = True, multi_intent_threshold: float = 0.5) -> Dict[str, Any]:
        ...
        intent_dict_cache = self.dal.get_intent_dict_cache()
    ...
    def _analyze_emotion(self, text: str) -> Dict[str, Any]:
        ...
        """
        text_lower = text.lower()
        emotion_keyword_cache = self.dal.get_emotion_keyword_cache()
        emotion_dict_cache = self.dal.get_emotion_dict_cache()
    ...
    def _match_business_intents(self, text: str, industry_code: str = "rental") -> List[Dict[str, Any]]:
        ...
        business_keyword_map = self.dal.get_business_intent_keyword_cache(industry_code)
        business_intent_list = self.dal.get_business_intent_cache(industry_code)
    ...
    def _extract_entities(self, text: str, industry_code: str = "rental") -> List[Dict[str, Any]]:
        ...
        # 1. 从数据库缓存读取通用实体关键词
        entity_keyword_map = self.dal.get_entity_keyword_cache()
        
        # 2. 读取业务实体关键词，合并到匹配规则（优先级更高）
        business_entity_keyword_map = self.dal.get_business_entity_keyword_cache(industry_code)
        business_entities = self.dal.get_business_entity_cache(industry_code)
