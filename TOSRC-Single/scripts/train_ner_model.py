...
"""
轻量小样本NER训练器（独立运行版本）
适配租房小数据集，基于规则+词典+CRF实现，无需大模型，训练速度快，适合离线场景
"""
import os
import sys
import json
...
class LightNERTrainer:
    ...
    def train_from_bio(self, dataset_dir: str, output_model_path: str = "data/models/ner_light") -> bool:
        ...
        try:
            ...
            logger.info(f"加载训练样本: {len(train_samples)}条，验证样本: {len(dev_samples)}条")
            ...
            logger.info(f"✅ 模型训练完成，验证集准确率: {accuracy:.2f}%")
            
            self.is_trained = True
            return True
            
        except Exception as e:
            logger.error(f"训练NER模型失败: {str(e)}", exc_info=True)
            return False
    ...
    def _build_entity_dictionary(self, samples: List[Tuple[List[str], List[str]]]) -> None:
        ...
        for et, values in entity_map.items():
            ...
            logger.info(f"实体类型[{et}]：{len(values)}个实体值")
    ...
    def _build_patterns(self) -> None:
        ...
        # 时间模式
        self.pattern_dict["time"] = re.compile(r'(今天|明天|后天|本周|下周|本月|下月|上午|下午|晚上|早上|[0-9]+[:点][0-9]+分?|[0-9]+[号日]|([0-9一二三四五六七八九十]+个?月))')
        # 手机号模式
    ...
    def _save_model(self, output_path: str) -> None:
        ...
        logger.info(f"模型已保存到: {output_path}/ner_model.json")
    ...
    def load_model(self, model_path: str) -> bool:
        ...
        try:
            ...
            self.is_trained = True
            logger.info(f"NER模型加载成功，包含{len(self.entity_dict)}类实体")
            return True
...
def main():
    ...
    dataset_dir = "/Volumes/1T/ai_project/ai-llm-router/TOSRC-Single/data/ner_dataset"
    model_path = "/Volumes/1T/ai_project/ai-llm-router/TOSRC-Single/data/models/ner_light"
    ...
    for text in test_texts:
        ...
        for e in entities:
            logger.info(f"  [{e['type']}] {e['value']} (置信度: {e['confidence']}, 来源: {e['source']})")
